...
let fileOverview = [
  { type: "input", name: "overview", message: "Description of the file" },
];
...
class Actions {
  ...
  createController() {
    ...
    return prompt(questions)
      ...
      .catch((err) =>
        message("error", `Error on create controller file: ${err}`)
      );
  }
    ...
    return prompt(questions)
      ...
      .then(() => {
        ...
        newQuestions.push({
          type: "confirm",
          name: "sync",
          message: "Want to sync the platform templates?",
        });
      ...
      .then(() => {
        ...
        const child = exec(`cd ${totalCmd.name} && npm install`).stderr.pipe(
          process.stderr
        );
  ...
  createHTMLLocalFiles(cmd) {
    ...
    return this.authAction(cmd, false, false).then((authCookie) => {
      ...
      return VTEXCMS.getHTMLTemplates()
        ...
        .then((templateNames) =>
          Promise.all(FS.createProjectHTML(templateNames, "HTML", cmd.name))
        )
        .then((files) =>
        ...
        .then((templateList) => VTEXCMS.getTemplateNames(templateList))
        .then((templateNames) =>
          Promise.all(FS.createProjectHTML(templateNames, "SUB", cmd.name))
        )
        .then((files) =>
          Promise.all(VTEXCMS.setTemplateContent(files, VTEXCMS.templates))
        )
        ...
        .then((templateList) => VTEXCMS.getTemplateNames(templateList))
        .then((templateNames) =>
          Promise.all(FS.createProjectHTML(templateNames, "SHELF", cmd.name))
        )
        .then((files) =>
          Promise.all(
            VTEXCMS.setTemplateContent(files, VTEXCMS.templates, true)
          )
        )
        .then((filesHTML) => Promise.all(FS.fillProjectHTML(filesHTML)))
  ...
  authAction(
    { email = null, account = null, site = "default" },
    checkPath = true,
    writeAuthStore = true
  ) {
    ...
    return prompt(questions)
      .then(({ account, email }) => {
        ...
        const authStore = VTEXID.checkAuthStore(
          this.account,
          this.email,
          writeAuthStore
        );
        ...
        return VTEXID.getEmailAccessKey(this.email)
          .then(() =>
            prompt({
              type: "input",
              name: "accesskey",
              message: "Enter the VTEX Access Key (with 6 digits)",
            })
          )
          .then(({ accesskey }) =>
            VTEXID.authenticateByEmailKey(this.email, accesskey)
          )
          .then((authCookie) => {
            if (writeAuthStore)
  ...
  uploadAssetsAction(cmd) {
    return this.authAction(cmd).then((authCookie) => {
      this._actionTitle(`Uploading Files (/files)`);

      return Promise.resolve(cmd);
      return Promise.all(VTEXCMS.setAssetFile(cmd)).then((responses) => {
  ...
  uploadDefaultAssetsAction(cmd) {
    return this.authAction(cmd).then((authCookie) => {
      ...
      return VTEXCMS.getRequestToken()
        .then((requestToken) =>
          Promise.all(VTEXCMS.defaultAssets(requestToken, cmd))
        )
        .then((responses) => {
          // responses.map(file => message('success', `Uploaded File ${file}`));
          this._successUpload(responses, "Asset");
  ...
  uploadHTMLAction(cmd) {
    return this.authAction(cmd).then((authCookie) => {
      ...
      return VTEXCMS.getHTMLTemplates()
        .then((templateList) =>
          Promise.all(VTEXCMS.setHTML(templateList, false, false, cmd))
        )
        .then((responses) => {
  ...
  uploadSubHTMLAction(cmd) {
    return this.authAction(cmd).then((authCookie) => {
      ...
      return VTEXCMS.getHTMLTemplates(true)
        .then((templateList) =>
          Promise.all(VTEXCMS.setHTML(templateList, true, false, cmd))
        )
        .then((responses) => {
          this._successUpload(responses);
          return cmd;
  ...
  uploadShelfAction(cmd) {
    return this.authAction(cmd).then((authCookie) => {
      ...
      return VTEXCMS.getHTMLTemplates(true, true)
        .then((templateList) =>
          Promise.all(VTEXCMS.setHTML(templateList, false, true, cmd))
        )
        .then((responses) => {
          this._successUpload(responses);
          return cmd;
  ...
  _successUpload(responses, typeFile = "Template") {
    responses.map(({ type, templateName, content, account }) => {
      message(
        type,
        `${
          type === "success" ? "Uploaded" : "Hold"
        } ${typeFile} ${templateName}`
      );
